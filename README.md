...
[![Project Status: Phase 1 Core Magnification](https://img.shields.io/badge/status-Phase%201%20Core%20Magnification-brightgreen)](specs/PRODUCT_STRATEGY.md)
...
> **Project Status:** Luminos has **completed Phase 0: Foundation**. Epics E01 (Project Scaffolding, Platform Traits & CI/CD), E02 (X11 Screen Capture & GPU Magnification), E03 (Input Tracking & Interactive Magnification), and E04 (Tauri Control Panel & Settings Persistence) are complete -- the first running Luminos application: a single-event-loop Tauri app hosting a transparent click-through wgpu magnification overlay plus a React control panel, with live full-screen magnification, typed tauri-specta IPC, config.toml settings persistence, and a system tray, on top of the Cargo workspace's 6 crates (trait definitions, X11 screen capture, GPU rendering pipeline, XInput2 input monitoring, cursor-follow viewport tracking, keyboard shortcuts, ArcSwap lock-free state management). Tests total ~446 workspace + 67 luminos-app Rust + 70 UI Vitest, backed by a GitHub Actions CI pipeline (8 active jobs) with dedicated platform (X11/Xvfb), GPU (Mesa llvmpipe), Tauri app, and tauri-driver E2E test jobs. Next up: Phase 1 (Core Magnification), starting with Epic E05.
...
| E4 | Tauri Control Panel & Settings Persistence | 3 weeks | **COMPLETE** -- transparent wgpu overlay + React control panel in a single Tauri event loop, tauri-specta IPC (7 commands + 2 events), config.toml persistence, system tray, ~446 workspace + 67 luminos-app Rust + 70 UI Vitest tests |
...
- The **magnification overlay** is a transparent, click-through wgpu surface on the Tauri (tao) window, controlled via x11rb and bypassing the webview entirely for GPU-level performance
...
| Window management | tao (via Tauri) + x11rb | Apache-2.0 (tao), MIT/Apache-2.0 (x11rb) |
...
|----------|--------|-------|
| Linux X11 | Phase 0 complete | Foundation (E01), X11 capture + GPU rendering (E02), interactive magnification (E03), and control panel + running app (E04) complete |
...
│   ├── luminos-types/         #   Shared data types (ScreenRect, DisplayInfo, CaptureFrame, ...)
...
│   └── luminos-app/           #   Tauri application shell (overlay + control panel, IPC, tray)
...
Luminos is in active development -- Phase 0 (Foundation) is complete and Phase 1 (Core Magnification) is beginning. Contributions are welcome in several areas:

- **Code** -- The Rust codebase has 6 crates and a running Luminos application: an interactive X11 magnification pipeline (capture, render, input monitoring, viewport tracking, keyboard shortcuts) plus a Tauri control panel with typed IPC, settings persistence, and a system tray; next up is Phase 1 (Core Magnification), starting with Epic E05

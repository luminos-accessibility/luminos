...
Runs `luminos-app` lib unit tests (incl. the offscreen Mesa-llvmpipe clear test). The subprocess integration tests that spawn the real Tauri binary (`app_lifecycle`, `managed_state`, `notifier_redraw`, `overlay_*`, `redraw_cadence`, `tray`, `ipc_events`, `live_magnification`) are currently **`#[ignore]`-quarantined** — see the blind-spot note below — so the CI run executes the lib unit tests only. Requires webkit2gtk-4.1 + libsoup-3.0 (to build the app), Xvfb, picom, `x11-utils` (`xprop`), `xdotool`, and Mesa drivers, plus a built `ui/dist` (run `pnpm --dir ui build` first so `overlay.html`/`index.html` exist). When run with `--run-ignored all` on a working desktop, the harness launches its OWN Xvfb + picom per test (no outer `xvfb-run`) and sets `GDK_BACKEND=x11` + `WEBKIT_DISABLE_COMPOSITING_MODE=1` + `WEBKIT_DISABLE_DMABUF_RENDERER=1` + `NO_AT_BRIDGE=1` per spawned child (the first three are required for GTK window realization + software GL under a headless Xvfb; `NO_AT_BRIDGE=1` is headless a11y-bus hygiene). Run single-threaded so each test's dedicated display does not contend.

> **Quarantine (DC-10/DC-13 blind spot).** On the Ubuntu-24.04 GitHub runner the spawned `luminos-app` boots too slowly under CI load and **intermittently** misses the harness boot-marker timeouts (≈20 s `wait_for_log`). The failure is flaky, not deterministic — in the cited run (27521471598) the job was `68 passed (4 flaky), 7 failed`: four spawn-tests booted fully and passed on retry (e.g. `app_logs_overlay_xid` at ~21 s, `overlay_surface` at ~14 s), so the app *does* reach the setup hook in passing attempts; seven failed all three tries. The flaky-pass times (≈14–25 s) straddle the 20 s timeout. The stall point in failing attempts is after the config-load log and before the first setup-hook log — *plausibly* during control-panel `WebKitWebView` construction, but this is a hypothesis, not confirmed (the cause is **not yet root-caused**). The same binary boots fine in the `test-e2e` job (launched via `WebKitWebDriver` under `xvfb-run`) and on local desktops, so the binary is correct; the problem is specific to this job's per-test Xvfb/picom/software-GL launch under runner load. Ruled out so far: GTK atk-bridge (NO_AT_BRIDGE=1 did not help), D-Bus session autolaunch (local no-bus sim boots fine), WebKit bwrap sandbox (wry/Tauri never enable it). The job has never been green since E04. All 19 boot-dependent spawn-tests are `#[ignore]`-quarantined (intermittency means any of them can fail on a given run, so a blanket skip is the only stable gate) and remain runnable locally with `cargo nextest run -p luminos-app --features "tauri custom-protocol ci_platform_tests" --run-ignored all`. Tracked in `specs/E04-tauri-control-panel/009-test-app-boot-flake/STORY.md`. **Do not un-ignore without a green CI run proving the flake is resolved.**
